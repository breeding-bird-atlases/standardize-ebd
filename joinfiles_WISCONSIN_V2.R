...
# change some of the file names (but the main auto downloads should be okay)
...
ebd <- read_ebd("ebd_US-WI_201501_201912_relNov-2021.txt", unique = FALSE)
...
sens <- read_ebd("SensitiveSpecies_ebd_relAug-2021_US-WI_2015-2019.txt", unique = FALSE)
...
atlasers <- read.csv("Wisconsin eBirders 20211216formatted.csv")
...
details <- file.info(list.files("C:/Users/nicho/Desktop/Non_EBD_Auto_Reports/joining", 
...
write.delim(ebdplussensitivepluszerocount, file = "Nov2021EBDmergedwithSensitiveSpeciesandZeroCountRecords.txt", sep="\t")
...
#################################################################################
# This section exports each table separately

library(plyr)
# note that loading plyr masks rename so if you rerun the code after this point the rename won't work
