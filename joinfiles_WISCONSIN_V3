...
ebd <- read_ebd("ebd_US-WI_201501_201912_relJan-2022.txt", unique = FALSE)
...
sens <- read_ebd("GOODsens_ebd_relJan-2022_US-WI_2015-2021.txt", unique = FALSE)
...
write.delim(ebdplussensitivepluszerocount, file = "test1EBDmergedwithSensitiveSpeciesandZeroCountRecords.txt", sep="\t")
...
library(plyr)
# note that loading plyr masks rename so if you rerun the above code after this point the rename won't work
...
# below is an example of adding the sensitive species onto the ebd, assuming you have run the code to the first set of hashes above.
...
#export file
write.delim(newebd, file = "yournewebd.txt", sep="\t")
